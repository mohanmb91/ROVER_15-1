...
public class ROVER_15 {
	...
	private void run() throws IOException, InterruptedException {
		...
		// we get data through 23.551.155.186:3000/api/science/excavate because
		// ours is excavators
		// if you go and check in communication class's getGlobalMap() function,
		// there you can see we have specified 'science/excavate'
		...
		while (true) {
			...
			// System.out.println("ROVER_15 equipment list results drive " +
			// equipment.get(0));
	...
	private List<Integer> getDirectionsToTargetLocation(Queue<Coord> targets, Communication com, String url,
		...
		if ((Math.abs(currentLocation.xpos - jackpotLocation.xpos) <= 3)
				|| (Math.abs(currentLocation.ypos - jackpotLocation.ypos) <= 3)) {
			out.println("GATHER");
		}
		// when reaches the jackpot location i.e., 79,51 GATHER it and add 3
		// corners of jackpot box as target
		if ((currentLocation.ypos == targetLocation.ypos) && (currentLocation.xpos == targetLocation.xpos)) {
			...
			out.println("GATHER");
			URL obj = null;
			// JSONArray data = com.convertScanMapTiles(currentLocation,
			// scanMapTiles);
			String responseStr = "";
			try {
	...
	private ArrayList<String> getEquipment() throws IOException {
		...
		// System.out.println("ROVER_15 incomming EQUIPMENT result - first
		...
		if (jsonEqListIn.startsWith("EQUIPMENT")) {
			while (!(jsonEqListIn = in.readLine()).equals("EQUIPMENT_END")) {
				...
				// System.out.println("ROVER_15 incomming EQUIPMENT result: " +
				// jsonEqListIn);
	...
	public void doScan() throws IOException {
		...
		if (jsonScanMapIn.startsWith("SCAN")) {
			while (!(jsonScanMapIn = in.readLine()).equals("SCAN_END")) {
				// System.out.println("ROVER_15 incomming SCAN result: " +
				// jsonScanMapIn);
		...
		// System.out.println("ROVER_15 convert from json back to ScanMap
		// class");
